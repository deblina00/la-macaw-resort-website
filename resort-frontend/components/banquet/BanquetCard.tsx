...
export default function BanquetCard({ banquet }: Props) {
  return (
    <Link href={`/banquets/${banquet.slug}`}>
      <div className="relative group overflow-hidden">
        ...
        <div className="relative h-[380px]">
          ...
          <div className="absolute top-5 left-5 border border-black/40 text-black text-xs tracking-widest px-4 py-1 backdrop-blur-sm bg-[#c6a85a]">
            {banquet.seatingCapacity?.max ||
              banquet.seatingCapacity?.floating ||
              "N/A"}{" "}
            GUESTS
          </div>
        </div>
        ...
        <div className="absolute bottom-0 p-6 text-white w-full">
          ...
          <p className="text-gray-300 text-sm max-w-xs leading-relaxed">
            Ideal for weddings, conferences and elegant celebrations at La Macaw
            Resort.
          </p>
        </div>
      </div>
